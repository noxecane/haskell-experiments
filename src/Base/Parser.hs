...
module Base.Parser
  ( Parser(runParser), runParser'
  , Result
  , (|>), (<|), (>>|), (|<<)
  , satisfy, nSatisfy, try -- * Basic building blocks
  , one, Base.Parser.max, Base.Parser.min, count -- * Count based combinators
  , seperatedBy, seperatedBy1
  , forever
  ) where
...
newtype Parser s a = Parser {
  runParser :: [s] -> Result s a -- ^ run parsing computation
}
...
instance Semigroup a => Semigroup (Parser s a) where
  Parser r1 <> Parser r2 = Parser merge
    where merge stream = case r1 stream of
...
instance Alternative (Parser s) where
  ...
  (Parser r1) <|> (Parser r2) = Parser trySecond
    where trySecond stream = case r1 stream of
...
satisfy name predicate = Parser isTrue
  where isTrue [] = (Left $ "No items for predicate: " ++ name, [])
        isTrue (p:ps)
...
try (Parser r) = Parser go
  where go stream = case r stream of
...
(Parser sep) |> (Parser p) = Parser go
  where go stream = case sep stream of
